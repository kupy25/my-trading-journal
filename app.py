...
# --- נתוני בסיס (לעדכן רק אם הפקדת/משכת כסף מהחשבון) ---
INITIAL_CASH = 44302.55        # ההון העצמי ההתחלתי איתו התחלת לעבוד ביומן
PORTFOLIO_START_VAL = 44302.55 # לחישוב תשואה כללית
...
try:
    ...
    # --- חישוב מזומן אוטומטי לחלוטין ---
    # א. רווח/הפסד שכבר נסגר (מוסיף או מוריד מהמזומן)
    realized_pnl = closed_trades['PnL'].sum()
    
    # ב. כסף שמושקע כרגע במניות (מוריד מהמזומן)
    current_invested_cost = open_trades['עלות כניסה'].sum()
    
    # ג. המזומן הפנוי: בסיס + רווח ממומש - עלות פוזיציות קיימות
    current_cash_live = INITIAL_CASH + realized_pnl - current_invested_cost

    # 4. חישוב שווי שוק (Live)
    market_val_total = 0
    live_list = []

    if not open_trades.empty:
        # איחוד לפי טיקר למקרה של כמה קניות באותה מניה
        ...
        for _, row in summary.iterrows():
            ...
            try:
                ...
                live_list.append({
                    'Ticker': t, 'כמות': row['Qty'], 'מחיר ממוצע': avg_cost,
    ...
    # --- חישובי שורה תחתונה ---
    total_portfolio_val = current_cash_live + market_val_total
    diff_from_start = total_portfolio_val - PORTFOLIO_START_VAL

    # --- SIDEBAR ---
    st.sidebar.header("⚙️ ניהול חשבון")
    st.sidebar.metric("מזומן פנוי (מחושב)", f"${current_cash_live:,.2f}")
    ...
    if c_entry > c_stop and c_stop > 0:
        risk_amount = total_portfolio_val * 0.01
    ...
    with t1:
        if live_list:
            ...
            pie_data = pd.concat([live_df[['Ticker', 'שווי פוזיציה']].rename(columns={'שווי פוזיציה': 'Value'}), 
                                 pd.DataFrame([{'Ticker': 'מזומן', 'Value': current_cash_live}])])
