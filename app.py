...
try:
    ...
    color_side = "#00c853" if diff >= 0 else "#ff4b4b"
    st.sidebar.markdown(f"<p style='color:{color_side}; font-size: 20px; font-weight: bold; margin-top:-10px;'>{'+' if diff >= 0 else ''}{diff:,.2f}$</p>", unsafe_allow_html=True)
    ...
    with t1:
        if not live_list == []:
            ...
            st.dataframe(
                df_display.style.map(color_pnl, subset=['רווח_דולרי', 'רווח_אחוז']),
