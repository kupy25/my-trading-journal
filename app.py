...
# --- מנגנון רענון אוטומטי (10 שניות) ---
# JavaScript שמרענן את כל הדף מבלי להסתמך על ספריות חיצוניות
components.html(
    """
    <script>
    setTimeout(function() {
...
try:
    ...
    # --- SIDEBAR (תצוגה קבועה ללא שגיאות) ---
    ...
    with st.sidebar.popover("🧮 מחשבון טרייד", use_container_width=True):
        ...
        calc_t = st.text_input("טיקר", key="sidebar_calc_ticker").upper()
        e_p = st.number_input("מחיר כניסה $", value=0.0, step=0.01, key="sidebar_calc_entry")
        s_p = st.number_input("סטופ לוס $", value=0.0, step=0.01, key="sidebar_calc_stop")
